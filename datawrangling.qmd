...
```{r}
df <- select(filter(macro_trends_imp, year == max(year)), life_exp, gni, yrs_sch)
df$cluster = factor(kmeans(select(ungroup(df), -Country), 4)$cluster)

library(plotly)
library(dplyr)
p <- plot_ly(df, x=~life_exp, y=~gni, 
z=~yrs_sch, color=~cluster) %>%
...
```{r}
plot_ly(data = macro_trends_with_clusters, 
        x = ~life_exp, y = ~gni, z = ~gdp, 
        color = ~cluster, 
        colors = c("red", "green", "blue", "purple"),
        marker = list(size = 5),
        type = "scatter3d",
        mode = "markers") %>%
  layout(scene = list(xaxis = list(title = "Life Expectancy"),
                      yaxis = list(title = "GNI"),
